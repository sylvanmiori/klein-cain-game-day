...
The same repository also hosts **413baseball.gameday.report**, a separate site for Steven Morris's 15U travel baseball team (4:13 Baseball, Spring TX) — built by Steven Miori, unaffiliated with Klein Cain. It has its own routes under `/baseball` (home, schedule, roster, stats, box-score upload), its own Perfect Game schedule scraper (Thursday automation) and bracket check (Saturday 11pm CT with hourly retries), and a D1-backed stats API fed only by manual box-score screenshot uploads. The box-score area is password-gated: the one-word password lives only in the Cloudflare Dashboard (Worker → Settings → Variables and Secrets, `BOXSCORE_PASSWORD`) and is never committed — see `cloudflare/.dev.vars.example` for local dev. Start with [`docs/baseball-site.md`](docs/baseball-site.md) for isolation, Cloudflare publish, Home UI, Maps links, and Open Graph rules; overview also in the "413baseball.gameday.report (4:13 Baseball)" section of [`docs/PROJECT-GUIDE.md`](docs/PROJECT-GUIDE.md). All baseball code is isolated from the football site.
