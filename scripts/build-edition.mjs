...
const schema = {
  ...
  properties:{
    ...
    headline:{type:'string',enum:['KLEIN CAIN GAMEDAY REPORT']}, dek:{type:'string'}, readHeadline:{type:'string'}, readBody:{type:'string'}, weatherLabel:{type:'string'}, weatherDetail:{type:'string'},
    players:{type:'array',minItems:6,maxItems:6,items:{type:'object',additionalProperties:false,required:['team','number','name','image','role','tag','rating','copy'],properties:{team:{type:'string'},number:{type:'string'},name:{type:'string'},image:{type:'string'},role:{type:'string'},tag:{type:'string'},rating:{type:'string'},copy:{type:'string'}}}}
  }
};

const prompt = `Research and write the Klein Cain varsity football game-day edition for ${JSON.stringify(game)}. Today is ${today}. Use web search and prefer official team schedules, MaxPreps, Rivals/On3, 247Sports, Hudl profiles and a current weather source. Verify records, rankings, player class/position, college interest, commitments, star ratings and prediction. Never infer a star rating or offer. If a service has no listing, say that plainly. Select exactly three players per team, with the strongest verified recruiting and returning-production stories. For each player, provide a direct public headshot or profile-image URL in image; use an empty string only when no confidently matched image exists. Keep every capsule concrete and under 55 words. Write like a restrained local sports editor: no hype filler, no rhetorical questions, no binary contrasts, no em dashes, no stacked fragments, no words such as leverage, robust, elevate, pivotal or game-changer. Use CAIN and the opponent name in uppercase as team labels. Set headline to exactly KLEIN CAIN GAMEDAY REPORT. The page already carries source links, so do not put article URLs elsewhere in the JSON.`;
