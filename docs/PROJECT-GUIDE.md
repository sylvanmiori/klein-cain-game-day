...
- **Box-score upload**: `/baseball/submit` posts a multipart image to `POST /api/baseball/boxscore`, gated by a one-word password checked against the `BOXSCORE_PASSWORD` worker secret (timing-safe compare, never logged; 401 on wrong, 503 when unconfigured). Workers AI vision (`@cf/meta/llama-3.2-11b-vision-instruct` via the `AI` binding) extracts structured batting/pitching lines as JSON for a confirmation screen; `POST /api/baseball/boxscore/confirm` writes them to D1 (new game INSERT, or DELETE+INSERT replacement for re-uploads; unknown player names grow the roster). `GET /api/baseball/stats` serves season aggregates (AVG/OBP/SLG/OPS, ERA/WHIP) to the stats pages and the game picker.
