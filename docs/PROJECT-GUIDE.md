...
Friday-night DCTF payloads are multi-megabyte (every row embeds HTML in `render` plus an unused `errorList`). Parsing that raw in the Worker cron exceeded the CPU budget and froze the feed mid-game on 2026-09-25. The ingest now strips those heavy fields with a linear, string-aware scan (`stripHeavyScoreFields` in `cloudflare/score.mjs`), extracts only the school's own rows without ever `JSON.parse`-ing the full games array (`extractSchoolGames`), and rejects payloads over `MAX_SCORE_PAYLOAD_BYTES` (8 MB) before parsing. A stress harness at `cloudflare/score-stress.mjs` (run with `node cloudflare/score-stress.mjs`, not part of `npm run test:score`) feeds synthetic 2.5/5/10 MB payloads through the same path; parsing cost is linear at roughly 10 ms per MB on server V8.

Watching the feed: cron attempts write ingest telemetry to KV under `<schoolId>:<slug>:ingest` with `lastAttempt`, `lastSuccess`, `lastError`, and `consecutiveFailures`. Failures and recoveries always record; steady-state successes heartbeat every 5 minutes to stay inside the free KV write quota. `GET /api/score/health?game=<slug>` returns that telemetry plus a `stale` flag, and `GET /api/score` itself now includes `stale` and `asOf`: a `live` score with no successful ingest for 10 minutes reports `stale: true`. To alert on a frozen feed, point any uptime monitor at `/api/score/health?game=<slug>` during game windows and alert when `stale` is true or `consecutiveFailures` reaches 3 (no alerting service is configured yet; the signal is ready for one).
...
## 413baseball.gameday.report (4:13 Baseball)

Added September 26, 2026. A second site on the same Worker and repository for Steven Morris's 15U travel baseball team (4:13 Baseball, Spring TX, 2026-2027 season) — built by Steven Miori as a favor; 4:13 Baseball is unaffiliated with Klein Cain and shares only hosting infrastructure with the football site. All baseball code is isolated and no football page, component, workflow, or config was changed. Baseball files: `app/baseball/`, `components/baseball/`, `content/baseball/`, `scripts/baseball-pg.mjs`, `scripts/baseball-bracket.mjs`, `scripts/baseball-seed.mjs`, `cloudflare/baseball-schema.sql`, `cloudflare/baseball-bracket.mjs`, `cloudflare/baseball-standings.mjs`, `docs/baseball-site.md`, `docs/baseball-brackets.md`, `docs/baseball-bracket-poller.md`, `docs/baseball-standings-poller.md`, `docs/baseball-d1.md`, `docs/baseball-pg-coverage.md`.

**Shared operating guide:** [`docs/baseball-site.md`](baseball-site.md) — isolation rules, Cloudflare publish path, Home UI conventions, venue Maps links, and the Open Graph trap (football metadata leaking into 413 link previews).

- **Hostname routing** lives in `cloudflare/worker.mjs`: requests to `413baseball.gameday.report` are rewritten to `/baseball/*` before the ASSETS fallthrough. The custom domain is declared in `cloudflare/wrangler.jsonc` routes. The `gameday.report` redirect and all football behavior are unchanged.
- **Publish path**: live 413 is Cloudflare only (`npm run build:cloudflare` then `npm run deploy:cloudflare`). A GitHub Pages publish does not update `413baseball.gameday.report`.
- **Branding**: Carolina blue (`#7BAFD4`), navy (`#12324e`), shield motif. Official shield PNGs live in `public/brand/baseball/` (`413-shield-48/96/192/512.png` + `413-shield-logo.png`); favicons are `app/baseball/icon.png` and `app/baseball/apple-icon.png`. Link-preview card is `public/brand/baseball/og.png` (1200×630).
- **Open Graph / iMessage previews**: `app/baseball/layout.tsx` and `app/baseball/page.tsx` must fully override root `openGraph` and `twitter` (title, description, url, siteName, image) with absolute `https://413baseball.gameday.report` values. Setting only `<title>` is not enough — Messages will still show Klein Cain football. After changing OG assets, re-share with a cache-bust query; Apple caches previews aggressively.
- **Home UI**: This Weekend is one flat card (time, field, pool, opponent, status) optimized for mobile width; see `docs/baseball-site.md` and mockup `docs/413-home-ui-mockup.png`.
- **Venue Maps**: Perfect Game `event_address` maps to `ScheduleTournament.address`; Home and Schedule expose a one-tap Google Maps link via `mapsSearchUrl` in `components/baseball/data.ts`.
- **Schedule source**: the Perfect Game team page (`orgid=69753&orgteamid=297202&Year=2027`). `npm run baseball:schedule` (`scripts/baseball-pg.mjs`) parses the TEAM SCHEDULE section into `content/baseball/schedule.json`. PG paywalls player stats (DiamondKast), so stats come only from manual uploads — the scraper never writes stats.
- **Stats database**: Cloudflare D1 `413baseball-stats`. Schema in `cloudflare/baseball-schema.sql`; roster seed via `npm run baseball:seed` (`scripts/baseball-seed.mjs`). Provisioning is manual because no wrangler auth exists in automation: `docs/baseball-d1.md` has the exact dashboard steps (create DB, paste the ID into `wrangler.jsonc` and uncomment the `d1_databases` block, apply schema, seed, `wrangler secret put BOXSCORE_PASSWORD`). The D1 binding stays commented until then; the API returns 503 with a plain message when it is absent, so deploys never break.
- **Box-score upload**: `/baseball/submit` is password-gated at the page level (enter once per visit, stored in sessionStorage; every write re-sends and the server re-checks; a 401 drops back to the gate). It has three modes: Upload (multi-file `images[]` screenshot input to `POST /api/baseball/boxscore` → vision parse → editable review → `POST /api/baseball/boxscore/confirm`), Manual entry (new or existing game, full editable batting/pitching grid), and Edit (load any game's lines via `GET /api/baseball/games/:id`, save via `PUT /api/baseball/games/:id/lines`, delete via `DELETE /api/baseball/games/:id` with a confirm step). All writes are gated by a one-word password checked against the `BOXSCORE_PASSWORD` worker secret (timing-safe compare, never logged; 401 on wrong, 503 when unconfigured; >10 failed attempts per IP in 5 min → 429 best-effort throttle). The password is never committed or written in docs: set it under Cloudflare Dashboard → Workers & Pages → `gameday-report` → Settings → Variables and Secrets (one word of Steven Morris's choice). `cloudflare/.dev.vars.example` documents the local-dev key; the real `.dev.vars` is gitignored. Until the secret exists the submit page says uploads are not enabled yet and the write endpoints fail closed. Workers AI vision (`@cf/meta/llama-3.2-11b-vision-instruct` via the `AI` binding) extracts GameChanger-format batting/pitching lines as JSON (jersey-number-first player matching, 1B/E/pitches/strikes, team result from the line score); `writeGameLines()` is the shared write path (new game INSERT, or DELETE+INSERT replacement; unknown player names grow the roster). `GET /api/baseball/stats` serves season aggregates (AVG/OBP/SLG/OPS, ERA/WHIP) to the stats pages and the game picker. Grid totals warnings are amber and never block saving — Steven Morris's word is final.
- **Automation** (`.github/workflows/`): `baseball-schedule.yml` runs Thursdays ~6pm CT and commits `content/baseball/schedule.json` when changed. `baseball-bracket.yml` runs hourly Saturday 11pm CT through Sunday 12pm CT and commits `content/baseball/bracket.json` once the PG bracket posts (URL pattern and extraction documented in `docs/baseball-brackets.md`), or `bracket-missing.json` as the alert signal if it never appears.
- **Bracket poller** (server-side, no agent in the loop): the Worker cron `*/15 * * * *` runs `pollBaseballBracket()` (`cloudflare/worker.mjs`), which fetches the weekend tournament's `Brackets.aspx` page from Perfect Game, parses it with the shared `cloudflare/baseball-bracket.mjs` module (also used by `scripts/baseball-bracket.mjs`), and writes a normalized snapshot to KV (`baseball:bracket:<event_id>`, telemetry at `baseball:bracket:<event_id>:ingest`) in the existing `SCORES` namespace. Cadence: every 15 minutes Saturday/Sunday 7am–11pm CT, hourly off-hours, silent skip when no tournament covers the weekend. Last-good semantics: the snapshot is written only after a successful fetch+parse, so a PG blip never blanks the page; failures record telemetry and rethrow so the dashboard shows them. `GET /api/baseball/bracket` serves the snapshot (404 when none yet); the Schedule page's `<BracketLive>` component (`components/baseball/bracket-live.tsx`) renders it with "Updated … · Source: Perfect Game" and falls back to build-time `bracket.json` on 404. Full detail: `docs/baseball-bracket-poller.md`.
- **Standings poller** (same cron, independent try/catch): `pollBaseballPoolStandings()` fetches `TournamentPoolStandings.aspx`, parses with shared `cloudflare/baseball-standings.mjs` (also used by `scripts/baseball-pg.mjs`; matches only `rptrPoolStandings` control ids so DiamondKast HTML on the page cannot corrupt cells), and stores last-good KV at `baseball:standings:<event_id>` (telemetry `:ingest`). `GET /api/baseball/standings` serves it; `/baseball/standings` (`<StandingsLive>`) renders pools as tables and falls back to `schedule.json` `pool_standings` on 404. Full detail: `docs/baseball-standings-poller.md`.
- The repo non-negotiables apply: never invent scores or statistics; schedule data comes only from PG parses and stats only from the owner's uploads.
- **Traps worth knowing (baseball):** (1) root `app/layout.tsx` OG defaults are football — always override on baseball layouts; (2) GitHub Pages ≠ 413 live; (3) `a { color: inherit }` can turn the active Home pill black unless active styles force white text; (4) D1 unbound → `/api/baseball/stats` 503 while empty-state UI is intentional.
