...
What automation still does not write is analysis. A generated edition has no players to watch, no keys and no recruiting notes, and its opening paragraph says only what is known and what updates later. Those sections stay empty until a person or a reviewed process fills them, because filling them automatically means inventing them. When that research is done, search the opponent roster as a whole for recruiting status before choosing capsules. Record the source and check date in `opponentRecruitingAudit`; every verified college commit must be a Player to watch and must name the college prominently. Do not confuse offers, interest or predictions with a commitment.
...
- Week 3 (Tomball, September 18) still carries an early-read paragraph and several player-stat capsules inherited from the original hardcoded page. The opponent recruiting roster was audited September 7 and Andres von der Meden's Texas A&M commitment is verified and included. Records, ranks, the prediction and the forecast refresh themselves; the remaining prose and statistics still need a fresh source check before game day.
